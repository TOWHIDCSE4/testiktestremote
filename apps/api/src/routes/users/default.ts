...
export const addPersonnel = async (req: Request, res: Response) => {
  const { fullName = "", factoryId, locationId, machineClassId } = req.body
  const email = `${fullName}@mail.com`
  const fullNameSplit = fullName.split(" ")
  const firstName = fullNameSplit.length > 0 ? fullNameSplit[0] : "Unnamed"
  const lastName = fullNameSplit.length > 1 ? fullNameSplit.slice(1).join() : ""
  const encryptPassword = CryptoJS.AES.encrypt(
    "defaultPassword",
    keys.encryptKey as string
  ).toString()
  ...
  const newUser = await Users.create(personnel)
