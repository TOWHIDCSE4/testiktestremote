...
const getTodayProductionTime = async ({
  locationId,
}: {
  locationId: string
}) => {
  ...
  const lastEndedControllerToday = await ControllerTimers.findOne({
    createdAt: { $gte: currentDateStart, $lte: currentDateEnd },
    endAt: { $exists: true },
  }).sort({
    endAt: -1,
  })
  const controllerBeforeLastEnded = await ControllerTimers.findOne({
...
export default getTodayProductionTime
